...
![img](/img/softmax-mnist.assets/NOHOCSA3A.png.webp#small)
...
由链式法则:
$$
\frac{\partial J(W)}{\partial W_{ij}} = -\frac{1}{N}\sum^N \frac{1}{P_y(x)} \frac{\partial P_y(x)}{\partial W\_{ij}}
$$
...
$$
\frac{\partial P_k(X)}{\partial W_{ij}} = \sum _t\frac{\partial P_k(x)}{\partial A_t} \cdot \frac{\partial A_t}{\partial W\_{ij}}
$$

考虑到当 $t \neq i$ 时, $\frac{\partial A_t}{\partial W_{ij}} = 0$, 所以:

$$
\frac{\partial P_k(X)}{\partial W_{ij}} = \sum _t\frac{\partial P_k(x)}{\partial A_t} \cdot \frac{\partial A_t}{\partial W\_{ij}} = \frac{\partial P_k(x)}{\partial A_i} \cdot \frac{\partial A_i}{\partial W\_{ij}}
$$
...
![image-20210716000330267](/img/softmax-mnist.assets/image-20210716000330267.png.webp#small)

下下来之后是个二进制文件, 下面介绍了二进制文件的构造.

![image-20210716000418636](/img/softmax-mnist.assets/image-20210716000418636.png.webp#small)

这是训练集输入, 从第16B开始, 每一个字节代表一个像素, 一张图有784个像素, 一共有60000个图.

![image-20210716000503830](/img/softmax-mnist.assets/image-20210716000503830.png.webp#small)
...
![2021-07-10](/img/softmax-mnist.assets/2021-07-10.png.webp#small)

我猜这是个5.

然后按照输出的格式, 读取第8个字节.

![2021-07-10-1](/img/softmax-mnist.assets/2021-07-10-1.png.webp#small)
...
![2021-07-15-3](/img/softmax-mnist.assets/2021-07-15-3.png.webp#small)
...
![img](/img/softmax-mnist.assets/4RBM1{TIEEXFDV72[2GK4FI.jpg.webp#small)
